...
#         }
        ...
        self.control = {
            'control' : list(np.unique(list(self.matches.values())))
        }
...
def comparison_plot(data, data_matched, treatment, feature, log_scale=False):
    plt.close()
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 2.5))
    fig.suptitle('Before vs. After Matching: ' + feature)

    sns.kdeplot(data=data[data[treatment]==1], x=feature, ax=ax1, log_scale=log_scale, label='treated').set()
    sns.kdeplot(data=data[data[treatment]==0], x=feature, ax=ax1, log_scale=log_scale, color='orange', label='untreated')
    ax1.set_title('Before Matching')
    ax1.set_xlabel(feature)
    ax1.legend(loc="upper left")

    sns.kdeplot(data=data_matched[data_matched[treatment]==1], x=feature, ax=ax2, log_scale=log_scale, label='treated').set()
    sns.kdeplot(data=data_matched[data_matched[treatment]==0], x=feature, ax=ax2, log_scale=log_scale, color='orange', label='untreated')
...
def comparison_stat_tests(data_matched, treatment, feature):
    x=np.array(data_matched[data_matched[treatment]==1][feature])
    y=np.array(data_matched[data_matched[treatment]==0][feature])
